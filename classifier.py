...
def classify_passcode():
    ...
    inputfile = 'ActHighRail_ep5_16k'
    model.load_weights(inputfile)
    
    newimg = eliminateCurve('pass_code3.png', 40)
    ...
    '''
    score = model.predict(np.reshape(p1[:, :, 0 : 1], (1, p1.shape[0], p1.shape[1], 1)) / 255, verbose = 1)
    str_mapped = mapping_list[np.argmax(score[0])]  
    str_predicted = str_predicted + str_mapped
    
    score = model.predict(np.reshape(p2[:, :, 0 : 1], (1, p2.shape[0], p2.shape[1], 1)) / 255, verbose = 1)
    str_mapped = mapping_list[np.argmax(score[0])]  
    str_predicted = str_predicted + str_mapped
    
    score = model.predict(np.reshape(p3[:, :, 0 : 1], (1, p3.shape[0], p3.shape[1], 1)) / 255, verbose = 1)
    str_mapped = mapping_list[np.argmax(score[0])]  
    str_predicted = str_predicted + str_mapped
    
    score = model.predict(np.reshape(p4[:, :, 0 : 1], (1, p4.shape[0], p4.shape[1], 1)) / 255, verbose = 1)
    str_mapped = mapping_list[np.argmax(score[0])]  
    str_predicted = str_predicted + str_mapped
